...
class Rbt_Project
{
    ...
    function create($nameIn, $aboutIn, $imageIn, $gogsLinkIn, $downloadLinksIn, 
        $dateStartIn, $dateEndIn, $languagesAndSkillsIn, $weblinkIn, $captionIn)
    {
        $this->name = $nameIn;
        $this->about = $aboutIn;
        $this->image = $imageIn;
        $this->gogsLink = $gogsLinkIn;
    ...
    function outputPortfolioLink($projectNumber)
    {
        ...
        if ($this->gogsLink != "")
        {
            $outString .= "                            <li>Gogs Link:".PHP_EOL;
            $outString .= "                                <strong> ".$this->gogsLink." </strong>".PHP_EOL;
